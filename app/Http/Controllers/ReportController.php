...
class ReportController extends Controller
{
    ...
    public function full_repayments(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            $pdf = PDF::loadView('loan_report.full_payment_pdf', compact('start_date',
                'end_date', 'data'));
...
public function trial_balance_consolidated(Request $request)
{
    ...
    public function trial_balance_pdf(Request $request)
    {
        ...
        if (!empty($end_date)) {
            //get disbursed loans within specified period and officer
            $data = GlAccount::orderBy('gl_code', 'asc')->get();
            $pdf = PDF::loadView('financial_report.trial_balance_pdf', compact('end_date', 'data', 'office_id'));
        ...
        if (!empty($start_date)) {
            if ($office_id != 0) {
                $data = LoanTransaction::where('transaction_type',
                    ...
                    $part_data = LoanTransaction::where('transaction_type',
                    'repayment')->where('payment_apply_to', 'part_payment')->where('reversed', 0)->where('office_id',
                    $office_id)->whereBetween('date',
                    [$start_date, $end_date])->with('loan')->with('office')->get();
                ...
                $data = LoanTransaction::where('transaction_type',
                'repayment')->where('payment_apply_to', 'full_payment')->where('reversed', 0)->whereBetween('date',
                [$start_date, $end_date])->with('loan')->with('office')->get();

                $part_data = LoanTransaction::where('transaction_type',
                    'repayment')->where('payment_apply_to', 'part_payment')->where('reversed', 0)->whereBetween('date',
                    [$start_date, $end_date])->with('loan')->with('office')->get();
...
public function repayments_report_details_csv(Request $request)
{
        ...
        if (!empty($start_date)) {
            if ($office_id != 0) {
                $data = LoanTransaction::where('transaction_type',
                    ...
                    $office_id)->whereBetween('date',
                    [$start_date, $end_date])->with('loan')->get();
            } else {
                $data = LoanTransaction::where('transaction_type',
    ...
    public function full_repayments_report_pdf(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            $pdf = PDF::loadView('loan_report.full_payments_pdf', compact('start_date',
                'end_date', 'data', 'office_id'));
            $pdf->setPaper('A4', 'landscape');
            return $pdf->download(trans_choice('general.repayment', 2) . ' ' . trans_choice('general.report',
                    1) . ".pdf");
    ...
    public function part_repayments_report_pdf(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            $pdf = PDF::loadView('loan_report.part_payments_pdf', compact('start_date',
                'end_date', 'part_data', 'office_id'));
            $pdf->setPaper('A4', 'landscape');
            return $pdf->download(trans_choice('general.repayment', 2) . ' ' . trans_choice('general.report',
                    1) . ".pdf");
        }
    ...
    public function reloans_report_pdf(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            $pdf = PDF::loadView('loan_report.reloans_pdf', compact('start_date',
                'end_date', 'reloans_data', 'office_id'));
            $pdf->setPaper('A4', 'landscape');
            return $pdf->download(trans_choice('general.repayment', 2) . ' ' . trans_choice('general.report',
                    1) . ".pdf");
        }
    ...
    public function new_loans_report_pdf(Request $request)
    {
        ...
        $office_id = $request->office_id;
            ...
            $pdf = PDF::loadView('loan_report.new_loans_pdf', compact('start_date',
                'end_date', 'new_loans', 'office_id'));
            $pdf->setPaper('A4', 'landscape');
            return $pdf->download(trans_choice('general.repayment', 2) . ' ' . trans_choice('general.report',
                    1) . ".pdf");
        }
...
$current_balance2 =  $opdebit - $opcredit;
        ...
        return view('financial_report.ledger_statement',
            compact('start_date',
                'end_date', 'data','credit','debit','current_balance','current_balance2','office_id', 'gl_account_id', 'opcredit','opdebit'));
    }
    ...
    public function disbursed_loans_pmec(Request $request)
    {
        ...
        return view('loan_report.disbursed_loans_pmec',
            compact('start_date',
                'end_date', 'data', 'office_id', 'loan_product_id', 'loan_officer_id', 'status'));
    }
